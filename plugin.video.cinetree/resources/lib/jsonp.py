...
def _create_args_list(args: str) -> list:
    ...
    #
    # FIXME: This relies on arguments of type text to have a space after a comma when
    #        there is a comma within that text.
    #        However, it will only be a problem when the text has more than one comma
    #        and the character directly following the comma is a normal letter. Special
    #        markdown character are disregarded. As well as text with a single comma and
    #        missing whitespace, because the text will than end with a '"'.
